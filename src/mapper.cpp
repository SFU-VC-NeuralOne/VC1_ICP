            ...
            uint8_t *pointer=nullptr, unsigned char unknown_cost_value=NO_INFORMATION)
{
    ...
    map_= map_.create(width_, height_, int);
    relative_map_= relative_map_.create(width_, height_, int);
...
{
    initial_angle = tf::getYaw(pose.getRotation());
  ...
  //cout<<"laser size"<<laser_size;
     ...
     for(int i=0 ;i<laser_size; i++)
    {
        ...
        polarToCartesian(r, a, scan_x, scan_y);
